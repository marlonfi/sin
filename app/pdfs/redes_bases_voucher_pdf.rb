class RedesBasesVoucherPdf < Prawn::Document
	...
	def data_header
	  ...
	  move_down @lineheight_y
	  text_box "Aportaciones por VOUCHER", :at => [@address_x, cursor]
	  move_down @lineheight_y
	...
	def make_tables(key, bases)
		...
		all_bases = []
	  bases.each do |base|
	  	total_voucher = Pago.por_fecha_base(@fecha,base.codigo_base).where(archivo: 'VOUCHER').sum(:monto)
	  	all_bases << [base.codigo_base, month_year(Date.parse(@fecha)),
	  							 number_to_currency(total_voucher, :unit => "S/. "),
	  							 number_to_currency(total_voucher/2 , :unit => "S/. ")]
	  end
	  #debugger
	  invoice_services_data = 
	    [["Cod. Base", "Mes",'Ingresos Voucher', "50%"]] +
	    all_bases +
	    [[" ", " ", " ", " ", " "]]

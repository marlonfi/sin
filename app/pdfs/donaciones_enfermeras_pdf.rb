class DonacionesEnfermerasPdf < Prawn::Document
	delegate :fecha_con_formato, :number_to_currency,  to: :@view
	...
	def data_header
	  ...
	  text_box "Asignaciones a enfermeras en el perido #{fecha_con_formato(@fecha_inicio)} a #{fecha_con_formato(@fecha_fin)}", :at => [@address_x, cursor]
	  move_down @lineheight_y
	  text_box "Total: #{@donaciones.count} asignaciones", :at => [@address_x, cursor]
	  move_down @lineheight_y
		move_cursor_to last_measured_y
	...
	def all_data
	  ...
	  @donaciones.each do |donacion|
	  	all_donaciones << [ donacion.enfermera.full_name,
	  								 donacion.motivo, number_to_currency(donacion.monto, :unit => "S/. "),fecha_con_formato(donacion.fecha_entrega), donacion.generado_por]
	  end
	  #debugger
	  invoice_services_data = 
	    [["Apellidos y Nombres","Motivo", 'Monto', "Fecha entrega", "Registrado por"]] +
	    all_donaciones +
	    [[" ", " ", " "," ", " "]]

class EntesPdf < Prawn::Document
	delegate :month_year, :number_to_currency, :date_and_hour,  to: :@view
	def initialize(entes, view)
		super()
		@entes = entes.sort_by { |obj| obj.red_asistencial.cod_essalud }
		@view = view
		@address_x = 35
		@lineheight_y = 12
		@font_size = 9
	...
	def data_header
	  ...
	  text_box "Listado de entes sin base", :at => [@address_x, cursor]
	  move_down @lineheight_y
	  
	  invoice_header_data = [
	    ["Dato", 'Entes sin Base'],
	    ["Generado", date_and_hour(Time.now)],
	    ["Total entes sin base", @entes.count]
	  ]
	...
	def all_data
	  ...
	  all_entes = []
	  @entes.each do |ente|
	  	numero_agremiados = ente.enfermeras.where(b_sinesss: true).count
	  	all_entes << [ente.red_asistencial.cod_essalud, ente.cod_essalud, ente.enfermeras.count, numero_agremiados]
	  end
	  #debugger
	  invoice_services_data = 
	    [["Red Asistencial", "Cod. Essalud Ente", "# Enfermeras", '# Agremiados']] +
	    all_entes +
	    [[" ", " ", " ", " "]]
